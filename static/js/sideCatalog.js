...
sideCatalog.getRangePanel = function() {
  ...
  rangePanel.innerHTML = '広い範囲（1/2オクテット）：'
  ...
  rangePanel.appendChild(document.createTextNode('狭い範囲（3/4オクテット）：'));
...
sideCatalog.transitionThread = function(cell, threadData, data) {
  ...
  if (!panelBypassId) {
    ...
    panelBypassId.appendChild(document.createTextNode('バイパスID：'));
