...
hiding.hidePost = function(linkSelf, board, thread, post) {
  ...
  var unhideHTML = '[非表示 ' + (post ? '投稿' : '投稿者') + ' ' + board + '/'
      + post + ']';
...
hiding.hideThread = function(linkSelf, board, thread) {
  ...
  unhideThreadButton.innerHTML = '[スレッドの非表示を解除する' + board + '/' + thread + ']';
...
hiding.buildHideMenu = function(board, thread, post, linkSelf, hideMenu) {
  ...
  filterNameButton.innerHTML = 'フィルター名';
  ...
  if (trip) {
    ...
    filterTripButton.innerHTML = 'トリップコードをフィルタリングする';
  ...
  if (labelId) {
    ...
    filterIdButton.innerHTML = 'フィルターID';
