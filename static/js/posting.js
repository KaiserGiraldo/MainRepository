...
posting.init = function() {
  ...
  posting.postCellTemplate = '<div class="innerPost"><h3 class="labelBoard"></h3><div class="postInfo title">'
      ...
      + 'class="linkSelf">No.</a> <a class="linkQuote"></a> <a class="linkEdit">[編集]</a> '
      + '<a class="linkHistory">[歴史]</a> <a class="linkFileHistory">[ファイル履歴]</a>'
      + ' <a class="linkOffenseRecord">[犯罪歴]</a>'
      + ' <span class="panelBacklinks"></span></div>'
      + '<div class="panelASN">ASN: <span class="labelASN"></span> </div>'
      + '<div class="panelBypassId"> バイパスID：<span class="labelBypassId"></span> </div>'
      + '<div>'
      + '<span class="panelIp"> <span class="panelRange">Broad'
      + 'range(1/2 octets): <span class="labelBroadRange"> </span> <br>'
      + '狭い範囲（3/4オクテット）： <span class="labelNarrowRange"> </span> <br>'
  ...
  posting.uploadCell = '<div class="uploadDetails"><a class="nameLink" target="blank">'
      ...
      + '<div> <a class="unlinkLink">[リンクを解除]</a>'
      + ' <a class="unlinkAndDeleteLink">[リンクを解除して削除]</a></div>'
      + '<a class="imgLink" target="blank"></a>';

  posting.sizeOrders = [ 'B', 'KB', 'MB', 'GB', 'TB' ];

  posting.guiEditInfo = '最終編集者は{$login}で{$date}です。';
...
posting.addRelativeTime = function(time) {
  ...
  if (delta > 2 * year) {
    content = Math.ceil(delta / year) + ' 往年';
  } else if (delta > 2 * month) {
    content = Math.ceil(delta / month) + ' 月前';
  } else if (delta > 2 * day) {
    content = Math.ceil(delta / day) + ' 日前';
  } else if (delta > 2 * hour) {
    content = Math.ceil(delta / hour) + ' 時間前';
  } else if (delta > 2 * minute) {
    content = Math.ceil(delta / minute) + ' 数分前';
  } else {
    content = 'たった今'
  }
