...
qr.setQr = function() {
  ...
  qrhtml += '素早い返信</span></th> </tr>';

  if (QRshowname) {
    qrhtml += '<tr><td colspan="2"><input id="qrname" type="text"';
    qrhtml += ' maxlength="35" autocomplete="off" placeholder="おなまえ"></td> </tr>';
  }

  qrhtml += '<tr><td colspan="2">';
  qrhtml += '<input id="qrsubject" type="text" maxlength="100"';
  qrhtml += 'autocomplete="off" placeholder="題　　名 ">';
  qrhtml += '</td>';
  qrhtml += '</tr>';

  qrhtml += '<tr><td colspan="2"><textarea id="qrbody" rows="5" placeholder="コメント">';
  qrhtml += '</textarea></td></tr> ';

  if (!textBoard) {

    if (api.mobile) {
      qrhtml += '<tr id="qrFilesButton"><td class="small">ファイル</td></tr>';
      qrhtml += '</tbody><tbody class="hidden" id="filesBody">';
    }

    qrhtml += ' <tr><td colspan="2"><div class="dropzone" id="dropzoneQr">';
    qrhtml += 'ファイルをドラッグしてアップロードするか、<br>ここをクリックしてファイルを選択します。</div>';
    ...
    qrhtml += '<label for="qrcheckboxSpoiler" class="spoilerCheckbox">ネタバレ</label></td> </tr>';
  ...
  if (!api.hiddenCaptcha) {
    ...
    qrhtml += ' value="リロード"> <span class="captchaTimer"></span></td></tr>';

    qrhtml += '<tr><td><input type="text" class="captchaField" ';
    qrhtml += 'id="QRfieldCaptcha" placeholder="答え"></td>';
    qrhtml += '<td><a href="/noCookieCaptcha.js" target="_blank" class="small">クッキーがない？</a></td></tr>';
  ...
  qrhtml += '<tr><td colspan="2">';
  qrhtml += '<input id="qrpassword" type="password" placeholder="パスワード"></td></tr>';
  ...
  qrhtml += '<button accesskey="s" id="qrbutton" type="button">返信する';
  qrhtml += '</td></tr>';
